from __future__ import print_function # for google use
import ctypes  # windows accessibility
import datetime
import json  # json file manipulation
import os  # raw functionality
import os.path
import pickle  # google authentication
import smtplib  # EMAIL
import time
import webbrowser
import platform  # system specification
from termcolor import colored  # coloured text
from urllib.request import urlopen  # handling google authentication
import pyjokes
import pyttsx3  # tts v - 3
import pytz  # adjust standard time stream in our local time stream
import requests  # handle HTTP requests
import speech_recognition as sr  # vidhayak
import wikipedia
import winshell  # recycle bin
from google.auth.transport.requests import Request  # dev console
from google_auth_oauthlib.flow import InstalledAppFlow
from googleapiclient.discovery import build
from mysql.connector import MySQLConnection
import csv  # comma separated values in graph
...
def virtualAssistant():
    ...
    def speak(text):
        engine = pyttsx3.init("sapi5")  # object creation # voice recognition and synthesis provided by Microsoft
    ...
    def take_command():
        ...
        if recognitionMode == 1:
            r = sr.Recognizer()
            with sr.Microphone() as source:  # source - input audio as a file
                print("Listening...")
                r.pause_threshold = 1
                audio = r.listen(source)  # store audio
            try:
                print("Recognizing...")
                query = r.recognize_google(audio, language='en-in')  # google engine has been used
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    'credentials.json', SCOPES)  # credential - every user has unique
                creds = flow.run_local_server(port=0)

            with open('token.pickle', 'wb') as token:
                pickle.dump(creds, token)

        service = build('calendar', 'v3', credentials=creds)  # calendar user authorize
    ...
    def activity(today, initTime, endTime):
        ...
        for i in range(len(importedData)):
            if len(importedData) != 0:
                dict[importedData[i][0]] = round(float(importedData[i][1]), 2) # time converted upto 2 decimals
        for i in dict:
            if i == today:
                print(dict[i])  # add time on same day
    ...
    def get_events(day, service):
        ...
        if not events:
            ...
            for event in events:
                start = event['start'].get('dateTime', event['start'].get('date'))  # 1 day format
                print(start, event['summary'])
                start_time = str(start.split("T")[1].split("-")[0])  # linked list
    ...
    if __name__ == "__main__":  # good programming practice
